...
// Use middleware for proper tenant isolation and role checking
checkAccess(['dentist'], true);
// Fetch clinic name for sidebar branding
$clinicName = 'Dental Clinic';
if (isset($_SESSION['tenant_id'])) {
    $clinicQuery = mysqli_query($conn, "SELECT clinic_name FROM tenants WHERE id = " . intval($_SESSION['tenant_id']));
    if ($clinicQuery && $clinicRow = mysqli_fetch_assoc($clinicQuery)) {
...
// Fetch dentist profile picture (safely check if column exists)
...
$notifBillingCount = 0;
// FIXED: Recognize both 'Completed' and 'Complete' for sidebar badges
$qBillCount = mysqli_query($conn, "SELECT a.id FROM appointments a JOIN lookup_statuses s ON a.status_id = s.id WHERE (s.status_name = 'Completed' OR s.status_name = 'Complete')" . getTenantFilter('a'));
if($qBillCount) $notifBillingCount = mysqli_num_rows($qBillCount);

$notifSchedule = 0;
$qSched = mysqli_query($conn, "SELECT id FROM appointments WHERE appointment_date = '$today' AND status_id = (SELECT id FROM lookup_statuses WHERE status_name = 'Pending')" . getTenantFilter());
if($qSched) $notifSchedule = mysqli_num_rows($qSched);

// Count paid appointments awaiting treatment records
$notifTreatmentCount = 0;
$qTreatment = mysqli_query($conn, "SELECT COUNT(*) as total FROM appointments a LEFT JOIN lookup_statuses s ON a.status_id = s.id LEFT JOIN treatment_records tr ON a.id = tr.appointment_id WHERE s.status_name = 'Paid' AND tr.appointment_id IS NULL" . getTenantFilter('a'));
if($qTreatment) {
...
// --- 3. DYNAMIC NEXT AVAILABLE SLOT LOGIC ---
...
for ($i = 0; $i < 30; $i++) {
    ...
    $checkQ = mysqli_query($conn, "SELECT a.appointment_time 
                                   ...
                                   AND a.tenant_id = " . intval($_SESSION['tenant_id']) . "
...
if (!empty($search)) {
    ...
    $sql_appt = "SELECT a.*, 
                 ...
                 LEFT JOIN lookup_statuses s ON a.status_id = s.id
                 WHERE a.tenant_id = " . intval($_SESSION['user_tenant_id']) . " AND ($whereSql)
    ...
    $sql_appt = "SELECT a.*, 
                 ...
                 LEFT JOIN procedures pr ON a.procedure_id = pr.id
                 LEFT JOIN lookup_statuses s ON a.status_id = s.id
                 WHERE a.tenant_id = " . intval($_SESSION['tenant_id']) . " AND a.appointment_date = '$filterDate'
...
if ($result_appt) {
    ...
    $trueTodayQuery = mysqli_query($conn, "SELECT a.id FROM appointments a LEFT JOIN lookup_statuses s ON a.status_id = s.id WHERE a.appointment_date = '$today' AND s.status_name NOT IN ('Paid', 'Rejected')" . getTenantFilter('a'));
    if ($trueTodayQuery) $countAppt = mysqli_num_rows($trueTodayQuery);
}

// FIXED: Standardized Billing Calculation for the clinic (Total Cost of Completed/Paid - Total Payments)
$sql_bill = "SELECT 
                (SELECT IFNULL(SUM(pr.standard_cost), 0) 
                 FROM appointments a 
                 JOIN procedures pr ON a.procedure_id = pr.id
                 LEFT JOIN lookup_statuses s ON a.status_id = s.id 
                 WHERE (s.status_name = 'Completed' OR s.status_name = 'Complete' OR s.status_name = 'Paid')" . getTenantFilter('a') . ") -
                (SELECT IFNULL(SUM(p.amount), 0)
                 FROM payments p
                 JOIN lookup_statuses s ON p.status_id = s.id
                 WHERE (s.status_name = 'Completed' OR s.status_name = 'Complete')" . getTenantFilter('p') . ") as total";
...
<head>
    ...
    <meta charset="utf-8"/>
    <title>Dentist Dashboard - <?php echo htmlspecialchars($clinicName); ?></title>
    ...
    <style>
        * { scroll-behavior: smooth; }
        ...
        .grid > div:nth-child(1) { 
            ...
            --color-from: rgba(59, 130, 246, 0.05);
            --color-to: rgba(255, 255, 255, 0);
        }
        .grid > div:nth-child(2) { 
            animation-delay: 0.2s;
            --color-from: rgba(249, 115, 22, 0.05);
            --color-to: rgba(255, 255, 255, 0);
        }
        .grid > div:nth-child(3) { 
            animation-delay: 0.3s;
            --color-from: rgba(168, 85, 247, 0.05);
            --color-to: rgba(255, 255, 255, 0);
        }
        .grid > div:nth-child(4) { 
            animation-delay: 0.4s;
            --color-from: rgba(34, 197, 94, 0.05);
            --color-to: rgba(255, 255, 255, 0);
        }
...
<div class="flex h-screen w-full overflow-hidden relative">
    ...
    <div id="sidebarOverlay" onclick="toggleSidebar()" class="fixed inset-0 bg-slate-900/60 z-30 hidden lg:hidden backdrop-blur-sm"></div>

    <aside id="sidebar" class="fixed lg:static inset-y-0 left-0 w-64 h-full flex flex-col bg-white dark:bg-[#1e293b] border-r border-slate-200 dark:border-slate-800 flex-shrink-0 z-40 hidden-mobile lg:translate-x-0 font-medium transition-colors duration-200">
        <div class="p-4 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-sm font-bold leading-tight text-slate-900 dark:text-white"><?php echo htmlspecialchars($clinicName); ?></h1>
        ...
        <nav class="flex-1 px-4 py-4 gap-2 flex flex-col overflow-y-auto font-black">
            <a class="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary/10 text-primary shadow-sm transition-all" href="dentist-dashboard.php">
            ...
            <a class="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-50 transition-colors" href="accounts.php">
    ...
    <main class="flex-1 overflow-y-auto h-full relative bg-[#f8fafc] dark:bg-background-dark text-slate-900 dark:text-white">
        <header class="lg:hidden flex items-center justify-between p-4 bg-white dark:bg-[#1e293b] border-b border-slate-200 dark:border-slate-800 sticky top-0 z-30 shadow-sm font-black">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-bold text-slate-900 dark:text-white"><?php echo htmlspecialchars($clinicName); ?></span>
            </div>
        ...
        <div class="p-6 md:p-10 max-w-[1600px] mx-auto flex flex-col gap-6 md:gap-10 animate-fade-in text-slate-900 dark:text-white">
            <header class="flex flex-col md:flex-row md:items-center justify-between gap-6 animate-fade-in">
                <div class="space-y-1">
                    <h1 class="text-4xl md:text-5xl font-black tracking-tight">Hello, Dr. <?php echo htmlspecialchars($displayFirstName); ?> 👋</h1>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 font-bold">
                <div class="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[24px] border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden text-sm shadow-md transition-all hover:scale-[1.02]">
                    ...
                    <p class="text-slate-500 text-[10px] font-black uppercase">Appointments today</p>
                ...
                <div class="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[24px] border border-slate-200 dark:border-slate-700 shadow-sm shadow-md transition-all hover:scale-[1.02]">
                    ...
                    <p class="text-slate-500 text-[10px] font-black uppercase">Pending billings</p>
                ...
                <div class="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[24px] border border-slate-200 dark:border-slate-700 shadow-sm shadow-md transition-all hover:scale-[1.02]">
                    ...
                    <p class="text-slate-500 text-[10px] font-black uppercase">New patients (today)</p>
                ...
                <div class="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[24px] border border-slate-200 dark:border-slate-700 shadow-sm ring-2 ring-primary/20 shadow-md transition-all hover:scale-[1.02]">
                    ...
                    <p class="text-primary text-[10px] font-black uppercase">Next available slot</p>
                    <h3 class="text-2xl md:text-3xl font-black mt-1 tracking-tighter tracking-tight"><?php echo $nextSlot; ?></h3>
                    <p class="text-[10px] text-slate-400 mt-1 font-bold italic transition-colors">PH time: <?php echo date('h:i A'); ?></p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-10">
                <div class="xl:col-span-2 flex flex-col gap-6">
                    ...
                    <form method="GET" class="px-2 mb-2 flex flex-col sm:flex-row gap-3 font-bold">
                        ...
                        <div class="flex flex-col gap-1 flex-1 text-[10px]">
                            ...
                            <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search visit name, procedure, or status..." class="h-10 w-full px-4 rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 text-sm font-bold focus:ring-primary shadow-sm transition-all tracking-tight">
                        </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-[32px] border border-slate-200 dark:border-slate-700 p-6 md:p-8 shadow-md flex flex-col gap-6 shadow-sm">
                        <?php if($resSchedule && mysqli_num_rows($resSchedule) > 0): ?>
                            ...
                            <div class="flex flex-col sm:flex-row gap-4 sm:gap-6 items-start animate-fade-in font-bold transition-all">
                                ...
                                <div class="flex-1 w-full p-4 md:p-6 rounded-2xl border-l-4 border-primary bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between shadow-sm transition-all hover:scale-[1.01] hover:bg-slate-100 dark:hover:bg-slate-900">
                                    ...
                                    <div class="size-10 md:size-12 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-400 border-2 border-white dark:border-slate-700 shrink-0 ml-4 shadow-sm transition-transform hover:scale-110 tracking-tight transition-colors uppercase">
                        ...
                        <?php else: ?>
                            <div class="text-center py-20 rounded-[32px] shadow-inner font-bold transition-all">
                                ...
                                <p class="text-slate-500 font-bold italic transition-colors text-[10px] uppercase">No active schedules found.</p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-4 font-black transition-all">
                    ...
                    <a href="schedule.php" class="h-14 w-full bg-primary text-white rounded-2xl font-black shadow-xl shadow-blue-500/30 hover:bg-blue-600 transition-all flex items-center justify-center gap-2 text-sm tracking-tight font-black">
                        <span class="material-symbols-outlined font-black">add</span> View appointment
                    </a>
    ...
    <div class="relative w-full max-w-[480px] bg-white dark:bg-slate-900 rounded-[32px] shadow-2xl overflow-hidden animate-fade-in duration-300">
        ...
        <div class="px-10 py-8 flex flex-col items-center text-center gap-8">
            <div class="space-y-3">
                ...
                <p class="text-slate-500 dark:text-slate-400 text-xs font-medium opacity-75">Your patient data will be saved.</p>
            </div>
...
<script>
    ...
    function updateDashboardUI(data) {
        ...
        // Update appointments table if visible
