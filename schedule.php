...
mysqli_query($conn, "CREATE TABLE IF NOT EXISTS `availability_blocks` (
    ...
    PRIMARY KEY (`tenant_id`, `block_date`)
)");

// Ensure tenant_id column exists for existing tables
$checkCol = mysqli_query($conn, "SHOW COLUMNS FROM `availability_blocks` LIKE 'tenant_id'");
if (!mysqli_fetch_assoc($checkCol)) {
    mysqli_query($conn, "ALTER TABLE `availability_blocks` ADD `tenant_id` INT(11) NOT NULL DEFAULT 1");
}

// 1. SECURITY & ACCESS via Middleware with Tenant Isolation
require_once 'backend/middleware.php';
checkAccess(['dentist', 'staff', 'patient'], true);
...
$blockedDates = [];
  $getBlocks = mysqli_query($conn, "SELECT block_date FROM availability_blocks WHERE tenant_id = " . getTenantValueForInsert());
...
$dashboardLink = '#'; // Dashboard removed - use mobile app
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_availability']) && $role === 'dentist') {
    ...
    if ($action === 'block') {
        mysqli_query($conn, "INSERT IGNORE INTO availability_blocks (tenant_id, block_date) VALUES (" . getTenantValueForInsert() . ", '$tDate')");
    } else {
        mysqli_query($conn, "DELETE FROM availability_blocks WHERE tenant_id = " . getTenantValueForInsert() . " AND block_date = '$tDate'");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_appt'])) {
    ...
    if ($role === 'assistant') {
        ...
        if ($role === 'patient') {
            ...
            $sql = "UPDATE appointments SET status_id = '$cancelStatID' WHERE id = '$delID' AND patient_id = '$userID'" . getTenantFilter();
        } else {
            $sql = "DELETE FROM appointments WHERE id = '$delID'" . getTenantFilter();
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_appointment'])) {
    ...
    if ($isUpdate) {
        $getPID = mysqli_query($conn, "SELECT patient_id FROM appointments WHERE id = '$editID'" . getTenantFilter() . " LIMIT 1");
        ...
        if ($isStaff && !empty($typedName)) {
            ...
            $findU = mysqli_query($conn, "SELECT id FROM users WHERE ((first_name='$f' AND last_name='$l') OR first_name='$typedName')" . getTenantFilter() . " LIMIT 1");
            if ($uRow = mysqli_fetch_assoc($findU)) {
                $targetPatientID = $uRow['id'];
            } else {
                $tempUser = "patient_" . time();
                mysqli_query($conn, "INSERT INTO users (tenant_id, first_name, last_name, username, role) VALUES (" . getTenantValueForInsert() . ", '$f', '$l', '$tempUser', 'patient')");
    ...
    if ($role === 'assistant') {
        ...
        if ($isUpdate && $role === 'dentist') {
            $sql = "UPDATE appointments SET tenant_id = " . getTenantValueForInsert() . ", patient_id='$targetPatientID', patient_name='$typedName', appointment_date='$date', appointment_time='$time', procedure_id=$finalProcID, status_id='$statID', downpayment_ref='$dpRef' WHERE id='$editID'" . getTenantFilter();
        } else {
            $tenantId = getTenantValueForInsert();
            $sql = "INSERT INTO appointments (tenant_id, patient_id, patient_name, appointment_date, appointment_time, procedure_id, status_id, downpayment_ref)
                    VALUES ($tenantId, '$targetPatientID', '$typedName', '$date', '$time', $finalProcID, '$statID', '$dpRef')";
        }
        if (mysqli_query($conn, $sql)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status_modal'])) {
    if ($role === 'dentist') {
        ...
        if ($newStatusName === 'Completed') {
            $dateCheck = mysqli_query($conn, "SELECT appointment_date FROM appointments WHERE id = '$apptID'" . getTenantFilter() . " LIMIT 1");
            $dCheck = mysqli_fetch_assoc($dateCheck);
            if ($dCheck && $dCheck['appointment_date'] !== $today) {
        ...
        $sql = "UPDATE appointments SET status_id = '$statID' WHERE id = '$apptID'" . getTenantFilter();
        if (mysqli_query($conn, $sql)) {
...
if (isset($_GET['edit_id']) && $role === 'dentist') { 
    ...
    $res = mysqli_query($conn, "SELECT a.*, pr.procedure_name as reason, s.status_name as status, u.first_name, u.last_name 
                                ...
                                WHERE a.id='$editID' AND a.tenant_id = " . intval($_SESSION['tenant_id']));
...
$sqlList = "SELECT a.*, u.username as acc_u, u.first_name, u.last_name, pr.procedure_name as reason, s.status_name as status,
            ...
            LEFT JOIN procedures pr ON a.procedure_id = pr.id
            LEFT JOIN lookup_statuses s ON a.status_id = s.id
            WHERE a.tenant_id = " . intval($_SESSION['tenant_id']);
...
// PARANOID: Always filter by tenant before executing
$sqlList .= " AND a.tenant_id = " . intval($_SESSION['tenant_id']);
...
<script>
    ...
    function openBackModal() { const m = document.getElementById('backModal'); const c = document.getElementById('backModalContent'); m.classList.remove('hidden'); m.classList.add('flex'); setTimeout(() => { m.classList.remove('opacity-0'); m.classList.add('opacity-100'); c.classList.remove('scale-95'); c.classList.add('scale-100'); }, 10); }
    function closeM(id) { const m = document.getElementById(id); m.classList.remove('opacity-100'); m.classList.add('opacity-0'); setTimeout(() => m.classList.add('hidden'), 300); }
    ...
    function closeViewModal() { document.getElementById('viewDetailsModal').classList.remove('opacity-100'); document.getElementById('viewDetailsModal').classList.add('opacity-0'); setTimeout(() => document.getElementById('viewDetailsModal').classList.add('hidden'), 300); }
</script>
</body>
</html>
