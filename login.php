            ...
            if (password_verify($password, $row['password'])) {
                ...
                if (isset($row['is_archived']) && $row['is_archived'] == 1) {
                    ...
                    if (!$skipClinicCheck) {
                        ...
                        $tenantCheckSql = "SELECT status, is_active, is_archived FROM tenants WHERE id = $tenantId LIMIT 1";
                        ...
                        if (!$tenantResult || mysqli_num_rows($tenantResult) === 0) {
                            ...
                            // CHECK: Clinic Archived - Prevent login if clinic is archived
                            if (isset($tenantRow['is_archived']) && $tenantRow['is_archived'] == 1) {
                                $_SESSION['login_attempts']++;
                                $error = "Your clinic has been archived and is no longer active. Please contact support.";
                                logLoginAttempt($row['username'], $row['email'], $row['id'], $row['role'], $row['tenant_id'], 'failed_clinic_archived', $error);
                            }
