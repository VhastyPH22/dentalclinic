...
// Security Check - only allow dentists
    ...
    if ($action === 'create_account') {
        ...
        if (empty($first_name) || empty($last_name) || empty($email) || empty($username) || empty($password) || empty($role)) {
            ...
            if (mysqli_num_rows($checkEmail) > 0) {
                ...
                // Insert user
                $insertUser = mysqli_query($conn, "INSERT INTO users (first_name, last_name, email, username, password, role, created_at) 
                                                    VALUES ('$first_name', '$last_name', '$email', '$username', '$hashed_password', '$role', NOW())");
        ...
        $user_id = intval($_POST['user_id'] ?? 0);
        if (mysqli_query($conn, "UPDATE users SET is_archived = 1 WHERE id = '$user_id' AND role IN ('dentist', 'assistant')")) {
        ...
        $user_id = intval($_POST['user_id'] ?? 0);
        if (mysqli_query($conn, "UPDATE users SET is_archived = 0 WHERE id = '$user_id' AND role IN ('dentist', 'assistant')")) {
...
// Fetch all dentist and assistant accounts with profile pictures
$sql = "SELECT u.id, u.first_name, u.last_name, u.email, u.username, u.role, u.created_at, p.profile_picture FROM users u LEFT JOIN patient_profiles p ON u.id = p.user_id WHERE u.role IN ('dentist', 'assistant') AND u.is_archived = $archiveFilter ORDER BY u.created_at DESC";
