...
$error = "";
$lockout_duration = 30; // Seconds to lock out user
        ...
        if ($result && mysqli_num_rows($result) === 1) {
            ...
            if (password_verify($password, $row['password'])) {
                ...
                if ($attempts_left <= 0) {
                    ...
                    $error = "Incorrect password. ($attempts_left attempts remaining)";
                }
            }
        } else {
            // USER NOT FOUND
        ...
        <section class="flex md:w-1/2 relative overflow-hidden flex-col justify-center items-center p-6 sm:p-8 md:p-12 text-center bg-gradient-to-br from-brandBlue via-slate-800 to-slate-900" data-purpose="branding-side">
            <div class="relative z-10 mb-4 sm:mb-6 md:mb-8" data-purpose="logo-container">
                <div class="relative inline-block">
                    ...
                    <img alt="San Nicolas Dental Clinic Logo" class="max-w-[180px] sm:max-w-[240px] md:max-w-[280px] lg:max-w-[320px] h-auto mx-auto drop-shadow-2xl relative z-10" src="assets/images/logo.png"/>
                </div>
            </div>
            ...
            <form action="" method="POST" class="space-y-5" data-purpose="auth-form">
                ...
                <div class="relative group">
                    ...
                    <div class="relative" data-purpose="password-input-container">
                        <input class="w-full px-4 py-2.5 sm:py-3 rounded-lg border-2 border-gray-200 focus:border-primary focus:ring-0 transition-all outline-none pr-12 font-medium hover:border-gray-300 text-sm group-focus-within:shadow-lg" id="password" name="password" placeholder="••••••••" required type="password" <?php echo (isset($_SESSION['lockout_time'])) ? 'disabled' : ''; ?>/>
                        <!-- Show/Hide Password Icon -->
                        <button aria-label="Toggle Password Visibility" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-primary transition-colors cursor-pointer" id="togglePassword" type="button" onclick="togglePasswordVisibility(event)">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between text-xs sm:text-sm gap-3">
                    ...
                    <a class="font-bold text-primary hover:text-blue-800 transition-colors relative inline-block" href="forgotpassword.php">Forgot Password?</a>
                </div>
                <!-- Login Button -->
                <button class="w-full bg-brandBlue hover:bg-blue-700 active:bg-blue-800 text-white font-bold py-2.5 sm:py-3 px-4 rounded-lg shadow-lg hover:shadow-xl transition-all mt-6 disabled:opacity-70 text-sm sm:text-base relative overflow-hidden" type="submit" <?php echo (isset($_SESSION['lockout_time'])) ? 'disabled' : ''; ?>>
                    <?php if(isset($_SESSION['lockout_time'])): ?>
                        <span class="inline-flex items-center gap-2">🔒 Locked for Security</span>
                    <?php else: ?>
                        <span class="inline-block">Log In</span>
                    <?php endif; ?>
                </button>
                <div class="text-center mt-6 pt-4 border-t border-gray-100">
                    <p class="text-xs sm:text-sm text-slate-600 mb-0">Don't have an account? 
                        <a class="font-bold text-primary hover:text-blue-800 transition-colors relative inline-block" href="register.php">Register here</a>
                    </p>
                </div>
            </form>
    ...
    <script data-purpose="event-handlers">
        ...
        function togglePasswordVisibility(e) {
            ...
            if (passwordInput.type === 'password') {
                ...
                passwordInput.style.animation = 'pulse-light 0.3s ease';
            } else {
        ...
        if (errorMsg && errorMsg.textContent.includes('wait')) {
            let countdownMatch = errorMsg.textContent.match(/(\d+)\s+seconds/);
            if (countdownMatch) {
                let secondsLeft = parseInt(countdownMatch[1]);
                const originalText = errorMsg.textContent.replace(/\d+\s+seconds/, '<span class="font-bold text-red-700 countdown-timer" id="countdownValue">' + secondsLeft + '</span> seconds');
                errorMsg.innerHTML = originalText;
                
                const countdownInterval = setInterval(() => {
                    secondsLeft--;
                    const countdownSpan = document.getElementById('countdownValue');
                    if (countdownSpan) {
                        countdownSpan.textContent = secondsLeft;
                        countdownSpan.style.animation = 'countdownPulse 0.5s ease';
                    }
