...
if (isset($_GET['view']) && $_GET['view'] === 'add' && $role !== 'patient') {
    ...
    if ($role === 'dentist' || $role === 'staff') {
        $view = 'edit';
        $targetID = $_GET['id'];
        // Verify access to this patient record
        requireTenantAccess('users', $targetID, 'patients.php');
    }
} elseif ($role === 'patient') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_patient'])) {
    ...
    $uCheck = mysqli_query($conn, "SELECT id FROM users WHERE (username = '$username' OR email = '$email')" . getTenantFilter());
    if(mysqli_num_rows($uCheck) > 0) $errors[] = "Username or Email is already registered.";

    $pCheck = mysqli_query($conn, "SELECT user_id FROM patient_profiles WHERE phone = '$phone'" . getTenantFilter());
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $sqlUser = "INSERT INTO users (tenant_id, role, first_name, last_name, email, username, password, is_archived, email_verified)
                        VALUES ($tenantId, 'patient', '$fname', '$lname', '$email', '$username', '$hashedPass', 0, 0)";
            if (!mysqli_query($conn, $sqlUser)) throw new Exception(mysqli_error($conn));

            $newID = mysqli_insert_id($conn);

            $sqlProfile = "INSERT INTO patient_profiles (tenant_id, user_id, dob, phone, address, occupation, marital_status, gender, chief_complaint)
                           VALUES ($tenantId, '$newID', '$bday', '$phone', '$addr', '$occup', '$marital', '$gender', '$complaint')";
            
            // Send verification email
            $verificationToken = bin2hex(random_bytes(32));
            $verifyExpiry = date('Y-m-d H:i:s', strtotime('+24 hours'));
            
            $tokenQuery = "UPDATE users SET email_verification_token = '$verificationToken', verification_token_expiry = '$verifyExpiry' WHERE id = '$newID'" . getTenantFilter();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_changes'])) {
    ...
    // Verify access to this patient record before allowing updates
    requireTenantAccess('users', $pid, 'patients.php');
    
    $uCheck = mysqli_query($conn, "SELECT id FROM users WHERE (username = '$username' OR email = '$email') AND id != '$pid'" . getTenantFilter());
    if(mysqli_num_rows($uCheck) > 0) $errors[] = "Username or Email already used by another account.";

    $pCheck = mysqli_query($conn, "SELECT user_id FROM patient_profiles WHERE phone = '$phone' AND user_id != '$pid'" . getTenantFilter());
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $sqlU = "UPDATE users SET first_name='$fname', last_name='$lname', email='$email', username='$username' WHERE id='$pid'" . getTenantFilter();
            ...
            if ($columnCheckResult && mysqli_num_rows($columnCheckResult) > 0) {
                $currentPictureQuery = mysqli_query($conn, "SELECT profile_picture FROM patient_profiles WHERE user_id = '$pid'" . getTenantFilter());
                if ($currentPictureQuery) {
                    $pictureData = mysqli_fetch_assoc($currentPictureQuery);
                    $preservePicture = $pictureData['profile_picture'] ?? '';
                }
                if ($preservePicture) {
                    $sqlP = "UPDATE patient_profiles SET dob='$bday', phone='$phone', address='$addr', occupation='$occup', marital_status='$marital', gender='$gender', chief_complaint='$complaint', profile_picture='$preservePicture' WHERE user_id='$pid'" . getTenantFilter();
                } else {
                    $sqlP = "UPDATE patient_profiles SET dob='$bday', phone='$phone', address='$addr', occupation='$occup', marital_status='$marital', gender='$gender', chief_complaint='$complaint' WHERE user_id='$pid'" . getTenantFilter();
                }
            } else {
                $sqlP = "UPDATE patient_profiles SET dob='$bday', phone='$phone', address='$addr', occupation='$occup', marital_status='$marital', gender='$gender', chief_complaint='$complaint' WHERE user_id='$pid'" . getTenantFilter();
            }
            ...
            if ($columnExists && mysqli_num_rows($columnExists) > 0) {
                $profileQuery = mysqli_query($conn, "SELECT profile_picture FROM patient_profiles WHERE user_id = '$pid'" . getTenantFilter());
                if ($profileQuery) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_patient'])) {
    ...
    // Verify access to this patient record
    requireTenantAccess('users', $deleteID, 'patients.php');
    if (mysqli_query($conn, "UPDATE users SET is_archived = 1 WHERE id = '$deleteID' AND role = 'patient'" . getTenantFilter())) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restore_patient'])) {
    ...
    // Verify access to this patient record
    requireTenantAccess('users', $restoreID, 'patients.php');
    if (mysqli_query($conn, "UPDATE users SET is_archived = 0 WHERE id = '$restoreID' AND role = 'patient'" . getTenantFilter())) {
...
if ($view === 'edit') {
    ...
    if ($columnExists && mysqli_num_rows($columnExists) > 0) {
        $sqlFetch = "SELECT u.*, p.dob as birthdate, p.phone, p.address, p.occupation, p.marital_status, p.gender, p.chief_complaint, p.profile_picture
                ...
                WHERE u.id = '$targetID'" . getTenantFilter('u') . "";
    } else {
...
if ($view === 'list') {
    ...
    if ($hasProfilePictureColumn) {
        $sqlList = "SELECT u.*, p.phone, p.profile_picture, p.gender, p.marital_status
                    FROM users u
                    LEFT JOIN patient_profiles p ON u.id = p.user_id
                      WHERE u.role = 'patient' AND u.is_archived = '$archiveVal'" . getTenantFilter('u') . "";
    } else {
