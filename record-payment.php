...
/**
 * 2. SECURITY & ROLE ACCESS via Middleware with Tenant Isolation
 */
...
if(!isset($_GET['view']) && ($role === 'dentist' || $role === 'staff')) {
    $view = 'list'; 
}

if ($role === 'staff' && ($view === 'add' || $view === 'edit')) {
    $view = 'list';
}

$dashboardLink = 'assistant-dashboard.php';
    ...
    $pQuery = "SELECT 
                ...
                (
                    (SELECT IFNULL(SUM(pr2.standard_cost), 0)
                     ...
                     WHERE a2.tenant_id = " . intval($_SESSION['tenant_id']) . " AND a2.patient_id = u.id
                    ...
                    (SELECT IFNULL(SUM(p.amount), 0)
                     ...
                     WHERE p.tenant_id = " . intval($_SESSION['tenant_id']) . " AND p.patient_id = u.id AND (s.status_name = 'Completed' OR s.status_name = 'Complete'))
               ...
               AND s.status_name != 'Paid'" . getTenantFilter('u') . "
...
if (isset($_POST['delete_payment']) && $role === 'dentist') {
    ...
    if (mysqli_query($conn, "DELETE FROM payments WHERE id = '$delID'" . getTenantFilter())) {
        ...
        if (empty($_POST['billing_info'])) {
            ...
            if ($date > $manilaDateStr) {
                ...
                if (isset($_POST['is_update']) && $_POST['is_update'] == '1') {
                    ...
                    $sql = "UPDATE payments SET amount='$amount', payment_date='$date', payment_type_id='$typeID', payment_method_id='$methodID', reference_no='$ref', status_id='$statusID' WHERE id='$updateID'" . getTenantFilter();
                    if (mysqli_query($conn, $sql)) { $success = true; $redirect = "updated"; }
                } else {
                    $check = mysqli_query($conn, "SELECT id FROM payments WHERE reference_no = '$ref'" . getTenantFilter());
                    if (mysqli_num_rows($check) > 0) {
                        $msg = "Error: Reference Number already exists."; $msgType = "error";
                    } else {
                        $sqlInsert = "INSERT INTO payments (tenant_id, patient_id, amount,
payment_date, payment_type_id, payment_method_id, reference_no, status_id) 
                                VALUES (" . getTenantValueForInsert() . ", '$pID', '$amount', '$date', '$typeID', '$methodID', '$ref', '$statusID')";
                ...
                if ($success) {
                    if (!empty($pID)) {
                        ...
                        $paidRes = mysqli_query($conn, "SELECT SUM(p.amount) as total_paid FROM payments p JOIN lookup_statuses s ON p.status_id = s.id WHERE p.patient_id = '$pID' AND (s.status_name = 'Completed' OR s.status_name = 'Complete')" . getTenantFilter('p'));
                        ...
                        while($row = mysqli_fetch_assoc($allRes)) {
                            ...
                            if (($totalPaid + 0.01) >= $runningTotalCost) {
                                mysqli_query($conn, "UPDATE appointments SET status_id = '$paidStatusID' WHERE id = '$targetID'" . getTenantFilter());
                            } else {
                                mysqli_query($conn, "UPDATE appointments SET status_id = '$compStatusID' WHERE id = '$targetID'" . getTenantFilter());
                            }
                        }
                    }
                            ...
                            <select name="billing_info" id="billingInfo" required class="w-full h-14 rounded-2xl bg-slate-50 dark:bg-slate-900 border-none px-5 focus:ring-primary font-black shadow-inner" onchange="handlePatientChange(this)">
                                ...
                                <?php foreach($patientsList as $p): ?>
                                    <option value="<?php echo $p['account_id']; ?>" 
                                        ...
                                        <?php echo htmlspecialchars($p['booked_name'] ?? ''); ?> - <?php echo htmlspecialchars($p['procedure_name'] ?? ''); ?> (₱<?php echo number_format($p['remaining_balance_for_this_appt'] ?? 0, 2); ?>) - <?php echo date('M d, Y', strtotime($p['appointment_date'])); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
