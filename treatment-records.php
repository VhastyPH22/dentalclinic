...
// --- ACCESS CONTROL via Middleware with Tenant Isolation ---
...
if ($targetPatientID > 0) {
    $pNameQuery = mysqli_query($conn, "SELECT first_name, last_name FROM users WHERE id = '$targetPatientID'" . getTenantFilter());
    // Verify access if dentist/staff is viewing another patient
    if ($role !== 'patient') {
        requireTenantAccess('users', $targetPatientID);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_treatment'])) {
    if ($role === 'patient') {
        ...
        if(empty($procID) || empty($date) || empty($formPatientID)) {
            ...
            if (!empty($recordID)) {
                $sql = "UPDATE treatment_records SET appointment_id='$appointmentID', procedure_id='$procID', treatment_date='$date', actual_cost='$cost', notes='$notes' WHERE id='$recordID'" . getTenantFilter();
                $notif = "updated";
            } else {
                $sql = "INSERT INTO treatment_records (tenant_id, appointment_id, patient_id, procedure_id, dentist_id, treatment_date, actual_cost, notes)
                        VALUES (" . getTenantValueForInsert() . ", '$appointmentID', '$formPatientID', '$procID', '$currentDentistID', '$date', '$cost', '$notes')";
                $notif = "added";
                ...
                $currentApptQuery = mysqli_query($conn, "SELECT patient_name FROM appointments WHERE id = '$appointmentID'" . getTenantFilter() . " LIMIT 1");
                ...
                if ($interval) {
                    ...
                    mysqli_query($conn, "INSERT INTO appointments (tenant_id, patient_id, patient_name, appointment_date, appointment_time, procedure_id, status_id)
                                        VALUES (" . getTenantValueForInsert() . ", '$formPatientID', '$patName', '$returnDate', '09:00:00', '$procID', '$confirmedStatusID')");
                }
            }
...
if (isset($_GET['edit']) && ($role === 'dentist' || $role === 'assistant')) {
    ...
    $editQuery = mysqli_query($conn, "SELECT t.*, t.actual_cost as cost FROM treatment_records t WHERE t.id = '$editID'" . getTenantFilter('t'));
...
$historySql = "SELECT t.*, u_pat.first_name, u_pat.last_name, pr.procedure_name, 
               ...
               WHERE t.tenant_id = " . intval($_SESSION['tenant_id']);
...
<script>
    ...
    function closeM(id) { 
        ...
        if(!el) return;
        const content = document.getElementById(id + 'Content') || el.querySelector('[id*="Content"]');
